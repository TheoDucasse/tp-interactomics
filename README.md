...
```
The EBV and EBV–human interactome data sets were generated by using a high-throughput Y2H system. 
```
...
```
C'est un générateur, elle permet de parcourir le rendu de la requette et de le modifier en liste plus digeste. La façon dont elle est construite permet qu'elle ne garde pas tout en mémoire et parcours juste le httpText sans encombrer la mémoire.
```
...
```
On obtient respectivement 113 et 64 proteines pour les jeux d'interactions EBV-Human et EBV-EBV.
```
...
### Construction du réseau d'interactionsS EBV/EBV
...
```
On obtient deux amas de proteines en interactions reliés par une liaison et autour on a un dizaine d'autres proteines liées 2 à 2. A noter que certaines protéines ont une action sur elles memes.
